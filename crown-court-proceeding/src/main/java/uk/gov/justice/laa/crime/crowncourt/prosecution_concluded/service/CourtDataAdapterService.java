...
public class CourtDataAdapterService {
    ...
    public List<Result> getHearingResult(ProsecutionConcluded prosecutionConcluded, UUID offenceId) {
        log.info(
                "Getting hearing result'{}' via court data adapter.",
                prosecutionConcluded.getHearingIdWhereChangeOccurred());

        final MultiValueMap<String, String> queryParams = new LinkedMultiValueMap<>();
        queryParams.add("publish_to_queue", "false");

        HearingResultResponse hearingResultResponse = courtDataAdaptorApiClient.getHearingResult(
                prosecutionConcluded.getHearingIdWhereChangeOccurred(), queryParams);
        log.info(
                "Completed getting hearing result '{}' via court data adapter.",
                prosecutionConcluded.getHearingIdWhereChangeOccurred());
