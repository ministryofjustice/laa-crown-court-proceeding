...
public class ResultDTOBuilder {
    ...
    public static List<Result> build(
            HearingResultResponse hearingResultResponse, ProsecutionConcluded prosecutionConcluded, UUID offenceId) {

        if (prosecutionConcluded == null
                || prosecutionConcluded.getProsecutionCaseId() == null
                || prosecutionConcluded.getDefendantId() == null
    ...
    private static List<JudicialResult> extractJudicialResults(
            Hearing hearing, ProsecutionConcluded prosecutionConcluded, UUID offenceId) {
        if (hearing == null || hearing.getProsecution_cases() == null) {
            return Collections.emptyList();
        }

        return hearing.getProsecution_cases().stream()
                .filter(prosecutionCase ->
                        isValidProsecutionCase(prosecutionCase, prosecutionConcluded.getProsecutionCaseId()))
                .flatMap(prosecutionCase -> prosecutionCase.getDefendants().stream())
                .filter(Objects::nonNull)
                .filter(defendant ->
                        defendant.getId() != null && defendant.getId().equals(prosecutionConcluded.getDefendantId()))
                .filter(defendant -> defendant.getOffences() != null)
                .flatMap(defendant -> defendant.getOffences().stream())
                .filter(offence -> offence != null && offence.getJudicial_results() != null)
                .filter(offence ->
                        Objects.nonNull(offence.getId()) && offence.getId().equals(offenceId))
