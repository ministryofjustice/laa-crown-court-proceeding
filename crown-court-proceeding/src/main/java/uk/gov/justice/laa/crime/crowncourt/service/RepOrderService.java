...
public class RepOrderService {
    ...
    List<String> grantedRepOrderDecisions =
            List.of(
                    Constants.GRANTED_FAILED_MEANS_TEST,
                    Constants.GRANTED_PASSED_MEANS_TEST,
                    Constants.GRANTED_PASSPORTED);
    List<String> grantedPassRepOrderDecisions =
            List.of(Constants.GRANTED_PASSED_MEANS_TEST, Constants.GRANTED_PASSPORTED);
    List<String> failedRepOrderDecisions =
            List.of(Constants.FAILED_IO_J_APPEAL_FAILURE, Constants.FAILED_CF_S_FAILED_MEANS_TEST);
        ...
        if (requestDTO.getCaseType() == CaseType.APPEAL_CC && reviewResult == ReviewResult.FAIL) {
            ...
            boolean isValidCaseType =
                    isValidCaseType(
                            ...
                            reviewResult);
            ccRepOrderDecision =
                    getDecisionByPassportAssessment(
                            requestDTO.getPassportAssessment(), isValidCaseType);
            if (ccRepOrderDecision == null) {
                ccRepOrderDecision =
                        getDecisionByFinAssessment(requestDTO, reviewResult, isValidCaseType);
            }
        }
    ...
    public String getDecisionByFinAssessment(
            CrownCourtDTO requestDTO, ReviewResult reviewResult, boolean isValidCaseType) {

        if (null != requestDTO.getFinancialAssessment()) {

            FullAssessmentResult fullAssessmentResult =
                    FullAssessmentResult.getFrom(
                            requestDTO.getFinancialAssessment().getFullResult());
            CurrentStatus fullAssessmentStatus =
                    requestDTO.getFinancialAssessment().getFullStatus();
            ...
            InitAssessmentResult initAssessmentResult =
                    InitAssessmentResult.getFrom(
                            requestDTO.getFinancialAssessment().getInitResult());
            CurrentStatus initAssessmentStatus =
                    requestDTO.getFinancialAssessment().getInitStatus();
            ApiHardshipOverview hardshipOverview =
                    requestDTO.getFinancialAssessment().getHardshipOverview();

            if (((initAssessmentResult == InitAssessmentResult.PASS
                                    && initAssessmentStatus == CurrentStatus.COMPLETE)
                            || (fullAssessmentResult == FullAssessmentResult.PASS
                                    && fullAssessmentStatus == CurrentStatus.COMPLETE)
                            || (hardshipOverview != null
                                    && (hardshipOverview.getReviewResult() == ReviewResult.PASS
                                            && hardshipOverview.getAssessmentStatus()
                                                    == CurrentStatus.COMPLETE)))
                    && isValidCaseType) {
                return Constants.GRANTED_PASSED_MEANS_TEST;
            } else if ((initAssessmentResult == InitAssessmentResult.FAIL
                            && initAssessmentStatus == CurrentStatus.COMPLETE)
                    || (fullAssessmentResult == FullAssessmentResult.FAIL
                            && fullAssessmentStatus == CurrentStatus.COMPLETE)) {
                return getDecisionByCaseType(
                        reviewResult, requestDTO.getCaseType(), requestDTO.getMagCourtOutcome());
            }
        }
        return null;
    }

    public String getDecisionByCaseType(
            ReviewResult reviewResult, CaseType caseType, MagCourtOutcome magCourtOutcome) {
        switch (caseType) {
            case COMMITAL -> {
    ...
    public String getDecisionByPassportAssessment(
            ApiPassportAssessment apiPassportAssessment, boolean isValidCaseType) {
        if (apiPassportAssessment != null) {
            PassportAssessmentResult passportResult =
                    PassportAssessmentResult.getFrom(apiPassportAssessment.getResult());
            if ((passportResult == PassportAssessmentResult.PASS
                            || passportResult == PassportAssessmentResult.TEMP)
    ...
    public boolean isValidCaseType(
            CaseType caseType, MagCourtOutcome magCourtOutcome, ReviewResult reviewResult) {
        return caseType == CaseType.INDICTABLE
    ...
    public void determineRepTypeByRepOrderDecision(
            CrownCourtDTO requestDTO, ApiCrownCourtSummary crownCourtSummary) {
        ...
        if ((grantedRepOrderDecisions.contains(repOrderDecision) && caseType == CaseType.APPEAL_CC)
                || (grantedPassRepOrderDecisions.contains(repOrderDecision)
                        && caseType == CaseType.COMMITAL)
    ...
    public void determineRepTypeByDecisionReason(
            CrownCourtDTO requestDTO, ApiCrownCourtSummary crownCourtSummary) {
    ...
    public ApiCrownCourtSummary determineRepOrderDate(CrownCourtDTO requestDTO) {
        ApiCrownCourtSummary crownCourtSummary = requestDTO.getCrownCourtSummary();
        String repOrderDecision = crownCourtSummary.getRepOrderDecision();
        if (StringUtils.isNotBlank(repOrderDecision)
                && crownCourtSummary.getRepOrderDate() == null) {
            switch (requestDTO.getCaseType()) {
                case INDICTABLE ->
                        crownCourtSummary.setRepOrderDate(
                                DateUtil.convertDateToDateTime(
    ...
    private LocalDateTime determineMagsRepOrderDate(
            CrownCourtDTO requestDTO, String repOrderDecision) {
        DecisionReason decisionReason = requestDTO.getMagsDecisionResult().getDecisionReason();
        ...
        if (MagCourtOutcome.COMMITTED_FOR_TRIAL.equals(requestDTO.getMagCourtOutcome())) {
            if (DecisionReason.GRANTED.equals(decisionReason)) {
                return DateUtil.convertDateToDateTime(
                        requestDTO.getMagsDecisionResult().getDecisionDate());
    ...
    public RepOrderDTO updateCCOutcome(CrownCourtDTO dto) {
        ...
        if (repOrderOutcomeCount == 0
                && null != dto.getCrownCourtSummary().getCrownCourtOutcome()
            ...
            if (null != request.getMagCourtOutcome()
                    && null != request.getEmstCode()
                ...
                if (null != evidenceFeeResponse.getEvidenceFee()) {
                    dto.getCrownCourtSummary()
                            .setEvidenceFeeLevel(
                                    EvidenceFeeLevel.getFrom(
                                            evidenceFeeResponse.getEvidenceFee().getFeeLevel()));
                }
            }
        }
