...
public class OffenceHelper {
    ...
    public List<OffenceSummary> getTrialOffences(List<OffenceSummary> offenceList, int maatId) {
        ...
        List<Integer> committalForTrialRefResults = courtDataAPIService.findResultsByWQTypeSubType(
                WQType.COMMITTAL_QUEUE.value(), COMMITTAL_FOR_TRIAL_SUB_TYPE);
        List<Integer> committalForSentenceRefResults = courtDataAPIService.findResultsByWQTypeSubType(
    ...
    private boolean isNewCCOffence(
        ...
        long newOffenceCount = courtDataAPIService.getOffenceNewOffenceCount(
                        caseId, offence.getOffenceId().toString())
                + courtDataAPIService.getWQOffenceNewOffenceCount(
                        caseId, offence.getOffenceId().toString());

        if (newOffenceCount > 0 && !hasCommittalResults(offence, offenceDTO, committalForSentenceRefResults)) {
            isNewCCOffence = true;
        }
    ...
    private boolean hasCommittalResults(
            OffenceSummary offence, List<OffenceDTO> offenceEntities, List<Integer> committalRefResults) {
        OffenceDTO offenceDTO = offenceEntities.stream()
                .filter(o -> o.getOffenceId() != null
                        && o.getOffenceId()
                                .equalsIgnoreCase(offence.getOffenceId().toString()))
                .findFirst()
                .orElse(null);
    ...
    private boolean hasResults(OffenceDTO offenceDTO, List<Integer> committalRefResults) {
        ...
        if (offenceDTO != null) {
            ...
            List<Integer> resultList =
                    courtDataAPIService.getResultCodeByCaseIdAndAsnSeq(offenceDTO.getCaseId(), asnSeq);
            List<Integer> wqResultList =
                    courtDataAPIService.getWqResultCodeByCaseIdAndAsnSeq(offenceDTO.getCaseId(), asnSeq);

            isCommittal =
                    Stream.concat(resultList.stream(), wqResultList.stream()).anyMatch(committalRefResults::contains);
        }
        return isCommittal;
    }
