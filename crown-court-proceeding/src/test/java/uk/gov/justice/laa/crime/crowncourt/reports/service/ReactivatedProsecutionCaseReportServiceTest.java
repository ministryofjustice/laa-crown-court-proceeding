...
class ReactivatedProsecutionCaseReportServiceTest {
    ...
    public void setUp() {
        ...
        reactivatedProsecutionCaseReportService.setEmailAddresses(
                List.of("test1@example.com", "test2@example.com"));
    }

    @Test
    void testGenerateReport_NoReactivatedCasesFound()
    ...
    @Test
    void testGenerateReport_ReactivatedCasesFound()
            throws IOException, NotificationClientException {

        List<ReactivatedProsecutionCase> reactivatedCases =
                List.of(new ReactivatedProsecutionCase());
        ...
        verify(emailNotificationService, times(1))
                .send(anyString(), anyList(), any(File.class), anyString());
        verify(reactivatedProsecutionCaseRepository, times(1))
                .updateReportingStatus(PROCESSED, PENDING);
    ...
    void testGenerateReport_ExceptionDuringFileGeneration()
        ...
        verify(emailNotificationService, never())
                .send(anyString(), anyList(), any(File.class), anyString());
        verify(reactivatedProsecutionCaseRepository, never())
                .updateReportingStatus(PROCESSED, PENDING);
    }
