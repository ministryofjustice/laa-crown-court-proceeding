...
class ProsecutionConcludedControllerTest {
    ...
    @Test
    void givenValidParameters_whenGetCountByMaatIdAndStatusIsInvoked_thenReturnCount() throws Exception {
        when(service.getCountByMaatIdAndStatus(TestModelDataBuilder.TEST_REP_ID, CaseConclusionStatus.PENDING.name()))
                .thenReturn(1L);
        mvc.perform(MockMvcRequestBuilders.get(String.format(COUNT_ENDPOINT_URL, TestModelDataBuilder.TEST_REP_ID)
