...
class CrownProceedingControllerTest {
    ...
    void processRepOrder_Success() throws Exception {
        var apiProcessRepOrderRequest = TestModelDataBuilder.getApiProcessRepOrderRequest(IS_VALID);
        ...
        when(crownProceedingService.processRepOrder(any(CrownCourtDTO.class))).thenReturn(processRepOrderResponse);
    ...
    void processRepOrder_RequestObjectFailsValidation() throws Exception {
        var processRepOrderRequest = TestModelDataBuilder.getApiProcessRepOrderRequest(!IS_VALID);
    ...
    void updateApplication_Success() throws Exception {
        var apiUpdateApplicationRequest = TestModelDataBuilder.getApiUpdateApplicationRequest(IS_VALID);
        ...
        when(crownProceedingService.updateApplication(any(CrownCourtDTO.class))).thenReturn(updateApplicationResponse);
    ...
    void givenARequestBodyIsMissing_whenUpdateCrownCourtIsInvoked_thenReturnServerError() throws Exception {
        mvc.perform(RequestBuilderUtils.buildRequestGivenContent(HttpMethod.PUT, "", UPDATE_CC_ENDPOINT_URL))
                .andExpect(status().is4xxClientError());
    }
    ...
    void givenAValidInput_whenUpdateCrownCourtIsInvoked_thenSuccessIsReturned() throws Exception {
        var apiUpdateCrownCourtRequest = TestModelDataBuilder.getApiUpdateCrownCourtRequest(IS_VALID);
        ...
        when(crownProceedingService.update(any(CrownCourtDTO.class))).thenReturn(updateResponse);
    ...
    @Test
    void givenAnInput_whenUpdateCrownCourIsInvokedAndValidationFails_thenBadRequestResponseIsReturned()
            throws Exception {
        var apiUpdateCrownCourtRequest = TestModelDataBuilder.getApiUpdateCrownCourtRequest(IS_VALID);
        var updateApplicationRequestJson = objectMapper.writeValueAsString(apiUpdateCrownCourtRequest);
        when(crownCourtDetailsValidator.checkCCDetails(any()))
                .thenThrow(new ValidationException(
                        CrownCourtDetailsValidator.CANNOT_HAVE_CROWN_COURT_OUTCOME_WITHOUT_MAGS_COURT_OUTCOME));
        mvc.perform(RequestBuilderUtils.buildRequestGivenContent(
                        HttpMethod.PUT, updateApplicationRequestJson, UPDATE_CC_ENDPOINT_URL))
                .andExpect(status().isBadRequest());
    }
