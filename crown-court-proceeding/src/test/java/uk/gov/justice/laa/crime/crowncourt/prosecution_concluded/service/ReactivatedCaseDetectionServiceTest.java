...
class ReactivatedCaseDetectionServiceTest {
    @InjectMocks ReactivatedCaseDetectionService reactivatedCaseDetectionService;
    @Mock private CourtDataAPIService maatCourtDataService;
    @Mock private ReactivatedProsecutionCaseRepository reactivatedProsecutionCaseRepository;

    @Test
    void
            givenCaseConcludedIsFalseAndPreviousCCOutcomeExists_whenGetProsecutionConcluded_thenRecordIsCreated() {
        ...
        when(reactivatedProsecutionCaseRepository.existsByMaatIdAndReportingStatus(
                        maatId, "PENDING"))
                .thenReturn(false);
    ...
    @Test
    void
            givenCaseConcludedIsTrueAndReactivatedCaseExists_whenGetProsecutionConcluded_thenRecordIsUpdated() {

        ProsecutionConcluded prosecutionConcludedRequest = getProsecutionConcluded(true);
        Integer maatId = prosecutionConcludedRequest.getMaatId();
        ReactivatedProsecutionCase reactivatedProsecutionCase =
                getReactivatedProsecutionCase(maatId);
        ...
        verify(reactivatedProsecutionCaseRepository, atLeast(1))
                .saveAndFlush(reactivatedProsecutionCase);
    }

    @Test
    void
            givenCaseConcludedIsFalseAndPreviousCCOutcomeDoesNotExist_whenGetProsecutionConcluded_thenRecordNotCreated() {
        ProsecutionConcluded prosecutionConcludedRequest = getProsecutionConcluded(false);
