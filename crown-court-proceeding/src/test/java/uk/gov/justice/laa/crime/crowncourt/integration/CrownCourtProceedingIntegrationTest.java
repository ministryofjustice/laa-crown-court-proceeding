...
class CrownCourtProceedingIntegrationTest extends WiremockIntegrationTest {
    ...
    @Test
    void givenEmptyOAuthToken_whenCreateAssessmentIsInvoked_thenFailsUnauthorizedAccess()
            throws Exception {
        mvc.perform(
                        RequestBuilderUtils.buildRequestGivenContent(
    ...
    void givenValidContent_whenApiResponseIsError_thenProcessRepOrderIsFails() throws Exception {
        var stubPath =
                "/api/internal/v1/assessment/ioj-appeal/repId/" + TEST_REP_ID + "/current-passed";
        ...
        mvc.perform(
                        RequestBuilderUtils.buildRequestGivenContent(
                                HttpMethod.POST,
                                objectMapper.writeValueAsString(apiProcessRepOrderRequest),
                                ENDPOINT_URL))
    ...
    @Test
    void givenValidEitherWayCaseTypeContent_whenProcessRepOrderIsInvoked_thenSuccess()
            throws Exception {
        MvcResult result =
                mvc.perform(
                                RequestBuilderUtils.buildRequestGivenContent(
                                        ...
                                        objectMapper.writeValueAsString(
                                                TestModelDataBuilder.getApiProcessRepOrderRequest(
                                                        Boolean.TRUE)),
                                        ENDPOINT_URL))
    ...
    void givenValidAppealCCContent_whenProcessRepOrderIsInvoked_thenSuccess() throws Exception {
        ...
        MvcResult result =
                mvc.perform(
                        ...
                        .andReturn();
        assertThat(result.getResponse().getContentAsString())
                .isEqualTo(objectMapper.writeValueAsString(processRepOrderResponse));
    }
    ...
    @Test
    void givenEmptyOAuthToken_whenUpdateApplicationIsInvoked_thenFailsUnauthorizedAccess()
            throws Exception {
        mvc.perform(
                        RequestBuilderUtils.buildRequestGivenContent(
    ...
    void givenInvalidContent_whenUpdateApplicationIsInvoked_thenFailsBadRequest() throws Exception {
        mvc.perform(
                        RequestBuilderUtils.buildRequestGivenContent(
                                HttpMethod.PUT,
                                objectMapper.writeValueAsString(
                                        TestModelDataBuilder.getApiUpdateApplicationRequest(
                                                Boolean.FALSE)),
                                ENDPOINT_URL))
                .andExpect(status().isBadRequest());
    }
    ...
    @Test
    void givenAValidUpdateApplicationContent_whenApiResponseIsError_thenUpdateApplicationIsFails()
            throws Exception {
        var stubPath = "/api/internal/v1/assessment/rep-orders";

        stubForOAuth();
        wiremock.stubFor(
                put(urlEqualTo(stubPath))
                        .willReturn(
                                WireMock.serverError()
                                        .withHeader(
                                                "Content-Type",
    ...
    @Test
    void givenValidContent_whenUpdateApplicationIsInvoked_thenUpdateApplicationIsSuccess()
            throws Exception {
    ...
    @Test
    void
            givenIndictableCaseTypeAndMagistratesOutcomeIsEmpty_whenUpdateIsInvoked_thenFailsBadRequest()
                    throws Exception {
        ApiUpdateCrownCourtRequest apiUpdateCrownCourtRequest =
                TestModelDataBuilder.getApiUpdateCrownCourtRequest(Boolean.TRUE);
        apiUpdateCrownCourtRequest.setCaseType(CaseType.INDICTABLE);
        apiUpdateCrownCourtRequest.setMagCourtOutcome(null);
        stubForOAuth();

        wiremock.stubFor(
                get(urlMatching("/api/internal/v1/assessment/rep-orders/cc-outcome/.*"))
                        .willReturn(
                                WireMock.ok()
                                        .withHeader(
                                                "Content-Type",
        ...
        mvc.perform(
                        RequestBuilderUtils.buildRequestGivenContent(
                                HttpMethod.PUT,
                                objectMapper.writeValueAsString(apiUpdateCrownCourtRequest),
                                ENDPOINT_URL + UPDATE_CC_URL))
                .andExpect(status().isBadRequest())
                .andExpect(
                        jsonPath("$.message")
                                .value(CANNOT_HAVE_CROWN_COURT_OUTCOME_WITHOUT_MAGS_COURT_OUTCOME));
    }
    ...
    void givenValidContent_whenUpdateIsInvoked_thenUpdateIsSuccess() throws Exception {
        ...
        mvc.perform(
                ...
                .andExpect(status().isOk())
                .andExpect(
                        jsonPath("$.crownCourtSummary.repOrderDecision")
                                .value(Constants.GRANTED_PASSED_MEANS_TEST))
                .andExpect(
                        jsonPath("$.crownCourtSummary.repType").value(Constants.CROWN_COURT_ONLY))
                .andExpect(
                        jsonPath("$.crownCourtSummary.evidenceFeeLevel")
                                .value(EvidenceFeeLevel.LEVEL1.getFeeLevel()))
                .andExpect(
                        jsonPath("$.crownCourtSummary.repOrderCrownCourtOutcome[0].outcome")
                                .value(CrownCourtOutcome.SUCCESSFUL.getCode()))
    ...
    private void stubForUpdateCC() throws Exception {
        ...
        wiremock.stubFor(
                post("/api/internal/v1/evidence/calculate-evidence-fee")
                        .willReturn(
                                WireMock.ok()
                                        .withHeader(
                                                "Content-Type",
                                        ...
                                        .withBody(
                                                objectMapper.writeValueAsString(
                                                        TestModelDataBuilder
                                                                .getApiCalculateEvidenceFeeResponse()))));
    }

    private void stubForIoJAppeal() throws JsonProcessingException {
        wiremock.stubFor(
                get("/api/internal/v1/assessment/ioj-appeal/repId/"
                                + TEST_REP_ID
                                + "/current-passed")
                        .willReturn(
                                WireMock.ok()
                                        ...
                                        .withBody(
                                                objectMapper.writeValueAsString(
                                                        TestModelDataBuilder.getIOJAppealDTO()))));
    }
}
