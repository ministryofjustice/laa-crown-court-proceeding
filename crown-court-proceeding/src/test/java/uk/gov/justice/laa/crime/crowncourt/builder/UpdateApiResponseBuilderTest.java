...
class UpdateApiResponseBuilderTest {
    ...
    void givenAValidInput_whenBuildIsInvoked_thenReturnApiUpdateCrownCourtOutcomeResponse() {
        ...
        List<RepOrderCCOutcomeDTO> repOrderCCOutcomeList = List.of(TestModelDataBuilder.getRepOrderCCOutcomeDTO(3, CrownCourtOutcome.PART_CONVICTED.getCode(),
                LocalDateTime.of(2022, 2, 7, 9, 1, 25)));
        ApiUpdateCrownCourtOutcomeResponse response = UpdateApiResponseBuilder.build(repOrderDTO, repOrderCCOutcomeList);
        ...
        softly.assertThat(crownCourtSummary.getRepOrderCrownCourtOutcome().get(0).getOutcome().getCode())
