...
class ProsecutionConcludedIntegrationTest extends WiremockIntegrationTest {
    ...
    void setup() {
        this.mvc = MockMvcBuilders.webAppContextSetup(this.webApplicationContext)
                ...
                .build();
        ProsecutionConcludedEntity prosecutionConcludedEntity = TestModelDataBuilder.getProsecutionConcludedEntity();
        repository.save(prosecutionConcludedEntity);
    }

    @Test
    void givenInvalidParameters_whenGetCountByMaatIdAndStatusIsInvoked_thenErrorIsThrown() throws Exception {
        mvc.perform(RequestBuilderUtils.buildRequestGivenContent(
                        HttpMethod.GET, "{}", String.format(COUNT_ENDPOINT_URL, "INVALID_REP_ID")))
                .andExpect(status().is4xxClientError());
    }

    @ParameterizedTest
    @MethodSource("countUrlProvider")
    void givenNonExistingMaatId_whenGetNewOffenceCountIsInvoked_thenZeroIsReturned(String url, int expectedCount)
            throws Exception {
    ...
    private static Stream<Arguments> countUrlProvider() {
        return Stream.of(
                ...
                Arguments.of(
                        String.format(COUNT_ENDPOINT_URL, TestModelDataBuilder.TEST_REP_ID) + "?status="
                ...
                Arguments.of(String.format(COUNT_ENDPOINT_URL, TestModelDataBuilder.TEST_REP_ID), 1));
    }
